...
- **[Ownership & Borrowing](language/ownership-borrowing.md)** -- `own`/`val`/`&`/`&mut` bindings, move checking, `region` arenas, `def drop` ([checker spec](language/ownership-checker-spec.md))
